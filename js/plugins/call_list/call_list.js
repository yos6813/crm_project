		...
		firebase.database().ref('reply/' + key).on('value', function (snapshot2) {
			firebase.database().ref('comment/').orderByChild('post').equalTo(key).on('value', function(snapshot3){
				firebase.database().ref('accept/' + key).on('value', function(snapshot4){
					...
					if(snapshot2.val().replyText != ''){
						check = '등록';
					}
					if(snapshot3.numChildren() > 0){
						comment = snapshot3.numChildren();
					}
					...
					$('#postList').each(function () {
						...
						$('#postList').append('<tr class="call_list" value="' + key + '">' +
							...
							'</td>' +
							 ...
							 '<a><img alt="image" class="acceptImgli img-circle" src="' +
							 	acceptImg + '"></a>' +
							 '<br/>' +
							 '<small>' + snapshot4.val().AcceptName + '</small>' +
							 '<br/>' +
							 '</td>' +
							...
							'</td>' +
							'</td>' +
							'<td class="project-title"><label class="badge badge-success">' + check + '</label>' + 
							'</td>' +
							'<td class="project-title">' + '<label class="badge badge-primary">' + comment + '</label></td>' +
							'</tr>');
				})
			})
						...
						$('.acceptImgli[src=""]').hide();
						$('.acceptImgli:not([src=""])').show();
...
$(document).ready(function () {
	...
	$(document).on('change', '#typeSelect', function () {
		...
		if (select == '전체') {
			...
			firebase.database().ref("qnaWrite/").orderByChild('type').equalTo(select).on('child_added', function (snapshot) {
				postList(snapshot.key);
			})
		}
	})
	...
	$('#radio2').click(function () {
		$('#postList').children('.call_list').remove();
		firebase.database().ref('qnaWrite/').orderByChild('status').equalTo('접수').on('child_added', function (snapshot) {
			postList(snapshot.key);
		});
	})
	...
	$('#radio3').click(function () {
		$('#postList').children('.call_list').remove();
		firebase.database().ref('qnaWrite/').orderByChild('status').equalTo('해결').on('child_added', function (snapshot) {
			postList(snapshot.key);
		})
	})
	...
	$('#radio5').click(function () {
		$('#postList').children('.call_list').remove();
		firebase.database().ref('qnaWrite/').orderByChild('status').equalTo('등록').on('child_added', function (snapshot) {
			postList(snapshot.key);
		})
	})
